# Syllabus DSL Specification — v1.3.1 (Normative)
...
This specification defines **DSL v1.3.1**.

DSL v1.3.1 **supersedes v1.3** by making all previously implicit enforcement rules explicit.
...
---
## 4.3 Topic Cardinality Constraints - Updated (DSL v1.3.1)

For each Unit, topic counts are constrained as follows.

### 4.3.1 Theory Topics
Theory contents is compulsory for each unit
...
### 5.2 Section Presence Rules (MANDATORY, PER UNIT) - Updated (DSL 1.3.1)

Let `(L, T, P, X)` be the declared credit structure.

For **each unit**:

- **Theory Content MUST exist**
...
- This document defines **DSL v1.3.1**
- Any change to:
...
Syllabi compiled under DSL v1.3.1 **MUST remain valid forever**.
