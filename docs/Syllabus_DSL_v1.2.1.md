# Syllabus DSL Specification — v1.2.1 (Normative)
...
This specification defines **DSL v1.2.1**.

DSL v1.2.1 **supersedes v1.2** by making all previously implicit enforcement rules explicit.
...
## 7. Articulation Matrices (CONDITIONAL, MANDATORY)

If programme outcomes exist:

- The course **MUST define** articulation matrices for:
  - CO–PO
  - CO–PSO
  - CO–SO

Rules:
- All Course Outcomes **MUST appear exactly once** as rows
- Columns **MUST match** the defined programme outcomes
- Allowed values:
  ```
  3, 2, 1, -
  ```
- `-` denotes no articulation
...
- This document defines **DSL v1.2.1**
- Any change to:
...
Syllabi compiled under DSL v1.2.1 **MUST remain valid forever**.
